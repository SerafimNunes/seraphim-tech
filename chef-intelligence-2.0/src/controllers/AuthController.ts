...
const loginSchema = z.object({
  ...
  // O campo enviado pelo cliente é 'senha'
...
class AuthController {
  ...
  public async login(req: Request, res: Response): Promise<Response> {
    try {
      ...
      // Mapeia o campo 'senha' do body para o campo 'senha_hash' que o Service espera
      const loginPayload = {
      ...
      const { token, usuario } = await this.service.login(loginPayload as any);

      // Retorna o token e os dados básicos do usuário (R12 - Segurança)
      return res.status(200).json({
        ...
        usuario: {
          ...
          // O objeto 'cargo' foi incluído pelo Sequelize no Service
