...
export interface ListarMovimentosFilters {
  ...
  unidade_id: number; // 🔑 CORREÇÃO CRÍTICA: Deve ser exigido para R4
}
...
export class EstoqueMovimentoService {
  ...
  public async listarMovimentos(
    ...
    // 🔑 R4: FILTRO CRÍTICO - Garante que apenas movimentos da unidade sejam retornados
    whereClause.unidade_id = filters.unidade_id; // 1. Filtro por ID do Produto
    ...
    try {
      ...
      const movimentos = await EstoqueRegistroMovimento.findAll({
        where: whereClause, // Adiciona a ordenação por data e ID para consistência
        order: [
          ['data_movimento', 'DESC'],
          ['id_movimento', 'DESC'],
        ],
  ...
  public async registrarMovimento(
    ...
    try {
      // 🔑 Validação R4: O campo `unidade_id` deve estar presente em `data` por causa da correção no Model.
