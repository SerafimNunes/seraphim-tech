...
export class ProducaoService {
  ...
  public async createProducao(
    ...
    try {
      ...
      const produto = await ItemEstoque.findByPk(id_produto_produzido, {
        attributes: ['id_produto', 'unidade_id', 'nome'], // 🔑 NOVO: Busca unidade_id
        transaction,
      });
      if (!produto) {
        throw new Error(
  ...
  public async deliverInsumos(
    ...
    try {
        ...
        include: [
          {
            ...
            attributes: [
              ...
              'estoque_atual',
            ], // 🔑 NOVO: Busca unidade_id e outros campos
          },
        ],
        transaction,
      })) as any[];
      ...
      for (const item of composicao) {
        ...
        // 🔑 CORREÇÃO TS2339 (Linha 318): Adicionado 'unidadeId' (3º argumento)
        const { custo_saida } = await this.estoqueService.saidaEstoque(
          insumo,
          qtd_total_saida.toNumber(),
          insumo.unidade_id, // 🔑 R4: Passa a unidade_id
          'SAIDA_PRODUCAO' as TipoMovimentoEstoque, // Tipo Movimento
  ...
  public async finishProduction(
    ...
    try {
      ...
      // 🔑 CORREÇÃO TS2339 (Linha 436): Adicionado 'unidadeId' (4º argumento)
      await this.estoqueService.entradaEstoque(
        produtoFinal,
        qtd_produzida_dec.toNumber(),
        novo_custo_unitario_producao.toNumber(),
        produtoFinal.unidade_id, // 🔑 R4: Passa a unidade_id
  ...
  public async createPerda(
    ...
    try {
      ...
      // 🔑 CORREÇÃO TS2339 (Linha 487): Adicionado 'unidadeId' (3º argumento)
      const { custo_saida } = await this.estoqueService.saidaEstoque(
        produto,
        qtd_perda_dec.toNumber(),
        produto.unidade_id, // 🔑 R4: Passa a unidade_id
