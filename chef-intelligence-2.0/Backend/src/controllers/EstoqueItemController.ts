...
// --- Esquemas de validação Zod (Corrigidos com colaborador_id) ---
...
const receberEstoqueSchema = z.object({
  ...
  referencia: z
    .string()
    .min(5, 'A referência do documento deve ser fornecida.'), // 🔑 CORREÇÃO TS2345 (Linha 157): Adicionado para atender ReceberEstoquePayload
  colaborador_id: z
    .number()
    .int()
    .positive('O ID do colaborador é obrigatório.'),
});
...
const saidaEstoqueSchema = z.object({
  ...
  referencia: z
    .string()
    .min(5, 'A referência do documento deve ser fornecida.'), // 🔑 CORREÇÃO TS2345 (Linha 193): Adicionado para atender SaidaEstoquePayload
  colaborador_id: z
    .number()
    .int()
    .positive('O ID do colaborador é obrigatório.'),
...
class EstoqueItemController {
  ...
  async receberEstoque(req: Request, res: Response): Promise<Response> {
    ...
    try {
      // 'data' agora inclui colaborador_id graças ao Zod Schema atualizado
      const data = receberEstoqueSchema.parse(req.body);

      const resultado = await this.service.receberEstoque(data); // 🔑 data agora é ReceberEstoquePayload válido
  ...
  async saidaEstoque(req: Request, res: Response): Promise<Response> {
    ...
    try {
      ...
      const { produto, custo_saida } = await this.service.saidaEstoque(data); // 🔑 data agora é SaidaEstoquePayload válido
