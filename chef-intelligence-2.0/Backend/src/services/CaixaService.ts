...
export default class CaixaService {
  public async abrirCaixa(
    colaborador_id_abertura: number, // 🔑 R4: Adiciona unidade_id
    unidade_id: number,
    saldo_inicial = 0,
  ): Promise<CaixaAttributes> {
    const novo = await Caixa.create({
      colaborador_id_abertura, // 🔑 R4 CORRIGIDO: Armazena o ID da Unidade
      unidade_id,
  ...
  public async fecharCaixa(
    id_caixa: number,
    colaborador_id_fechamento: number, // 🔑 R4 CORRIGIDO: Adiciona unidade_id para filtro de segurança
    unidade_id: number,
  ): Promise<{ relatorio: any }> {
    const caixa = await Caixa.findOne({
      where: {
        id_caixa,
        unidade_id, // 🔑 R4 CORRIGIDO: Filtra pela Unidade (Segurança/Isolamento)
      },
    });
    ...
    if (caixa.status_caixa === 'FECHADO')
      throw new Error('O caixa já está fechado.'); // TODO: A lógica de cálculo final e total_vendas/despesas deve ser feita aqui (buscando lançamentos e vendas).
   ...
   * 🔑 CORRIGIDO: Busca o caixa ativo da unidade.
   ...
   * Lista caixas abertos da Unidade (R4)
   */
   ...
   * Lista lançamentos (movimentos) da Unidade (R4)
   */
