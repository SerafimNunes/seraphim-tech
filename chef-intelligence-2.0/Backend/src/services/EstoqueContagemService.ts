// backend/src/services/EstoqueContagemService.ts
...
export class EstoqueContagemService {
    ...
    public async registrarContagem(
        ...
        const { id_produto, estoque_contado, colaborador_id, observacoes } = payload;
        ...
        try {
            ...
            transaction = await connection.transaction({
                isolationLevel: (Sequelize as any).Transaction.ISOLATION_LEVELS.SERIALIZABLE,
            ...
            const estoqueTeorico = new Decimal(produto.estoque_atual as unknown as string);
            const estoqueFisico = new Decimal(estoque_contado);
            const custoUnitario = new Decimal(produto.preco_custo_unitario as unknown as string);
            ...
            const registroData: EstoqueRegistroContagemCreationAttributes = {
                ...
                colaborador_id, // 🔑 NOTA: Adicione a unidade_id aqui se o modelo EstoqueRegistroContagem a exigir.
            };
            await EstoqueRegistroContagem.create(registroData, { transaction });

            // 5. 🔑 DELEGAÇÃO: AJUSTE DE ESTOQUE (Movimento)
            if (discrepancia.isZero()) {
                ...
                // 🔑 CORREÇÃO TS2339
            } else if (discrepancia.greaterThan(0)) {
            ...
            // 7. Retorno dos dados para o Controller
