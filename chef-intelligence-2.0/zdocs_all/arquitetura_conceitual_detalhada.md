...
Objetivo:
Criar um Dashboard BI interativo, similar ao Power BI, que permita gerar múltiplas telas ou páginas, cada uma dedicada a uma área da empresa (Financeiro, Operacional, RH/Pessoas), mostrando KPIs estratégicos em tempo real, com filtros e drill-down para análise detalhada.

Comportamento esperado do Frontend:

Dashboard modular, com uma tela inicial resumida e telas detalhadas por área.

Cada KPI exibido em visualizações interativas (gráficos de barra, linha, tabela dinâmica, KPI cards).

Permitir drill-down, filtros dinâmicos e exportação de relatórios.

Cada tela deve ser responsiva e navegável, permitindo alternar rapidamente entre áreas e relatórios.

Estrutura de telas e KPIs:

Tela	Área	KPIs Chave (R10)	Dados Fonte	Visualização sugerida
Tela 1 – Financeiro	Financeiro	CMV Real-Time, Margem de Contribuição (MC), MCMP, Break-Even Point (R8), Margem de Lucro Real, Ticket Médio	Vendas (Receita), Estoque (Custo), Compras (CMP)	KPI Cards, Gráficos de Barra e Linha, Tabelas Dinâmicas, Drill-down por produto/unidade
Tela 2 – Operacional	Operacional	Tempo Médio de Preparo (TMP), Desperdício (R7), Curva ABC de Vendas, Índice de Acurácia de Estoque	Produção (Tempo/Descarte), Vendas	Gráficos de Linha e Área, Mapas de Calor de Desperdício, Curvas ABC, Drill-down por setor e período
Tela 3 – RH / Pessoas	RH/Pessoas	Taxa de Rotatividade (Turnover), Tempo para Contratar, Custo por Contratação, Clima Organizacional (Módulo 9)	RH, Dados operacionais correlacionados com Desperdício/Erros de Venda	KPI Cards, Gráficos de Barra e Pizza, Tabelas Dinâmicas, Drill-down por cargo, unidade ou período

Regras adicionais para desenvolvimento:

Cada KPI deve ser atualizado em tempo real ou próximo do tempo real, respeitando as regras do AnaliseService.

O frontend deve permitir criação de novas telas e relatórios de forma dinâmica, caso novos KPIs sejam adicionados.

Os gráficos devem ter filtros dinâmicos por período, unidade, produto e categoria, com possibilidade de exportação (PDF/Excel).

O Dashboard deve ser modular, permitindo que novas áreas ou indicadores sejam adicionados sem refatoração do sistema.
